...
32 marketing jobs where I haven't found an OSS pick I'm comfortable recommending yet. Closed tools tend to own these today (Clay for ICP discovery, Dreamdata for attribution, Profound for LLM citation tracking, Surfer for content updates, Modash for creator outreach). If you know of an OSS tool that fits, or you're building one, open a PR and I'll take a look.

Competitor monitoring · Customer interviews · ICP discovery · Market research · Campaign briefs · Customer personas · Long blog posts · Updating old posts · Repurposing content · Case studies · Keyword research · On-page SEO audits · Site crawls · Tracking LLM mentions · Internal linking · Social scheduling · Long video to shorts · Creator outreach · Community replies · Ad copy variants · Ad creative variants · Ad budget management · Landing pages from a brief · A/B test analysis · Welcome email sequences · Email list segmentation · Newsletter writing · Inbox triage · Deliverability monitoring · Analytics summaries · Attribution summaries · Performance reports.
...
<details>
<summary>⚠️ <strong>Watchlist</strong> (7)</summary>
...
- **[Beehiiv MCP](https://github.com/danvega/beehiiv-mcp-server)** — Closes the loop for newsletter drafting agents on Beehiiv — hand an LLM-written issue directly to Beehiiv for preview and send instead of pasting into the editor.
- **[FireGEO](https://github.com/firecrawl/firegeo)** — Firecrawl starter for tracking brand and competitor citations inside ChatGPT, Claude, and Perplexity answers. Best when GEO visibility is the KPI, not web pages.
- **[GEO AI Agent](https://github.com/brightdata/geo-ai-agent)** — Tracks how a brand shows up in ChatGPT, Perplexity, and Gemini answers over time. Bright Data scraper under a CrewAI loop. Category is still unsettled, so expect this entry to move as better tools ship.
- **[Ghost MCP](https://github.com/MFYDev/ghost-mcp)** — Closes the loop on the Ghost substrate entry — drafting skills push finished posts directly into Ghost without manual copy-paste. Lightly maintained (flag: watch for staleness).
